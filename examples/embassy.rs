...
fn main() -> ! {
    ...
    // TODO we have to conjure the wifi device here... surely there is a way to mix the svc stuff with embassy?
    // perhaps wifi device should contain embedded_svc::wifi::Configuration instead and have the svd traits impled on it instead?
...
async fn net_task(stack: &'static Stack<WifiDevice>) {
    esp_println::println!("About to run net task");
    stack.run().await
}
...
async fn task(stack: &'static Stack<WifiDevice>) {
    ...
    loop {
        esp_println::println!("In main task");
        ...
        let remote_endpoint = (Ipv4Address::new(192, 168, 0, 24), 8080);
        println!("connecting...");
